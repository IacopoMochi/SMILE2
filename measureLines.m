...
%     mF_LW = mF_LW/L;
    ...
    elseif strcmp(PSDmodel,'Integral')
        model = @Integral;
        %Rv = abs((-N/2:N/(N-1):N/2)*ps);
        edf=[find(freq==freqx(1),1),find(freq==freqx(end),1)];
        beta0 = [sqrt(nanmean(mF_LWx(1:FN))-nanmean(mF_LWx(Lx-LN:Lx))), 1/CF, (nanmean(mF_LWx(Lx-LN:Lx))),Alpha];
        f = @(beta,Rv,edf,mF_LWx) sum((Integral(beta,freqL,edf)-mF_LWx).^2);
        Options = optimset('MaxFunEvals',MFE,'MaxIter',MI);
        betaf = fminsearch(@(beta) f(beta,freqL,edf,mF_LWx),beta0,Options);
        %edf=[find(freq==freqx(1),1)-1,find(freq==freqx(end),1)];
        %disp(beta0)
        %disp(betaf)
        %betaf = beta0;
        
        %mF_LW_fit = model(betaf,freqL,[edf(1),length(freq)]);
